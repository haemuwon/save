...
include_once(dirname(__FILE__) .'/pbkdf2.compat.php'); // 7.4 호환용 파일 인클루드 @221024
...
//한페이지에서 페이징 따로 쓸때(두번째페이징변수)
function get_paging2($write_pages, $cur_page, $total_page, $url, $add="")
{
	if(!$write_pages) $write_pages = 5;

	//$url = preg_replace('#&amp;page=[0-9]*(&amp;page=)$#', '$1', $url);
	$url = preg_replace('#&amp;page2=[0-9]*#', '', $url) . '&amp;page2=';
...
function set_cookie($cookie_name, $value, $expire)
{
	...
	setcookie(md5($cookie_name), base64_encode($value), strtotime('+'.$expire.'days'), "/", G5_COOKIE_DOMAIN ); //@200912
}
...
class html_process {
	...
	function run()
	{
		...
		if($config['cf_count_member']==1){

		/** 현재 접속멤버 카운트 **/
		$tmp_sql = " select count(*) as cnt from {$g5['login_table']} where mb_id = '{$_SESSION['ss_mb_id']}' ";
		$tmp_row = sql_fetch($tmp_sql);

		if ($tmp_row['cnt']) {
			$tmp_sql = " update {$g5['login_table']} set lo_ip = '{$_SERVER['REMOTE_ADDR']}', lo_datetime = '".G5_TIME_YMDHIS."', lo_location = '{$g5['lo_location']}', lo_url = '{$g5['lo_url']}' where mb_id = '{$_SESSION['ss_mb_id']}' ";
			sql_query($tmp_sql, FALSE);
		} else {
			$tmp_sql = " insert into {$g5['login_table']} ( lo_ip, mb_id, lo_datetime, lo_location, lo_url ) values ( '{$_SERVER['REMOTE_ADDR']}', '{$_SESSION['ss_mb_id']}', '".G5_TIME_YMDHIS."', '{$g5['lo_location']}',  '{$g5['lo_url']}' ) ";
			sql_query($tmp_sql, FALSE); 
		...
		if ($tmp_row['cnt']) {
			$tmp_sql = " update {$g5['login_table']} set mb_id = '{$_SESSION['ss_mb_id']}', lo_datetime = '".G5_TIME_YMDHIS."', lo_location = '{$g5['lo_location']}', lo_url = '{$g5['lo_url']}' where lo_ip = '{$_SERVER['REMOTE_ADDR']}' ";
			sql_query($tmp_sql, FALSE);
		} else {
			$tmp_sql = " insert into {$g5['login_table']} ( lo_ip, mb_id, lo_datetime, lo_location, lo_url ) values ( '{$_SERVER['REMOTE_ADDR']}', '{$_SESSION['ss_mb_id']}', '".G5_TIME_YMDHIS."', '{$g5['lo_location']}',  '{$g5['lo_url']}' ) ";
			sql_query($tmp_sql, FALSE); 
