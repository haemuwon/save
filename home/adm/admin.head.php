...
function print_menu2($key, $no)
{
    ...
    for($i=1; $i<count($menu[$key]); $i++)
    {
		...
		if($menu[$key][$i][1]=='DB관리'){
		 $str .= '<li class="gnb_2dli '.$check_gnb_grp_style.'"><a href="'.$menu[$key][$i][2].'" class="gnb_2da '.$gnb_grp_style.' '.$gnb_grp_div.'" data-text="'.$menu[$key][$i][1].'" target="_blank">'.$menu[$key][$i][1].'</a></li>';
		}
		else{
