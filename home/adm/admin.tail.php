...
$(function(){
	...
	$('.gnb_1da').bind('click', function(){
		...
		if(gnb_parent.hasClass('on')) { 
			...
			$('#gnb .on').not('.check').removeClass('on');
			$('#gnb .check').parents('li').addClass('on');
		} else { 
			gnb_parent.addClass('on');
			gnb_parent.siblings().removeClass('on');
		}
		return false;
	});

	$('#gnb .check').addClass('on').parents('li').addClass('on');
});
