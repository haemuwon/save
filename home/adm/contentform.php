...
//그룹추가
if(!sql_query(" select co_group from {$g5['content_table']} limit 1 ", false)) {
	$sql = " ALTER TABLE `{$g5['content_table']}`  ADD `co_group` VARCHAR( 20 ) NOT NULL AFTER `co_id` ";
	sql_query($sql, false);
}
...
//뷰 레벨 추가
if(!sql_query(" select co_view from {$g5['content_table']} limit 1 ", false)) { 
	sql_query(" ALTER TABLE `{$g5['content_table']}`
					ADD `co_view` tinyint(4) NOT NULL DEFAULT '1' AFTER `co_content`", true);
}
...
<div class="tbl_frm01 tbl_wrap">
	...
	<tr>
		...
		<td>
			...
			<?php if ($w == 'u') { ?><a href="<?php echo G5_BBS_URL; ?>/content.php?co_id=<?php echo $co_id; ?>" class="btn_frmline">페이지확인</a><?php } ?>
		</td>
	</tr>
	<tr>
		<th>보기 권한</th>
		<td><?php echo get_member_level_select('co_view', 1, 10, $co['co_view']) ?></td>
	</tr>
	<tr>
	...
	<tr>
		<th scope="row"></th>
		<td class="btn_list"> <a href="#" onclick="$(this).next().slideToggle(); return false;">치환자 안내</a><div style="display:none;">
		<table>
			<colgroup>
			...
			<td>로그인 링크 (비로그인 상태에서만 출력)</td>
			</tr>
			<tr>
			<th><strong>{{LOGOUT}}</strong></th>
			<td>로그아웃 링크 (로그인 상태에서만 출력)</td>
			</tr>
			<tr>
			<th><strong>{{JOIN}}</strong></th>
			<td>회원가입 링크 (비로그인 상태에서만 출력)</td>
			</tr>
			<tr>
			<th><strong>{{SETTING}}</strong></th>
			<td>회원정보 수정 페이지 링크 (로그인 상태에서만 출력. 관리자는 관리자페이지->회원관리에서만 정보수정이 가능합니다)</td>
			</tr>
			<tr>
			<th><strong>{{ADMIN}}</strong></th>
			<td>관리자 페이지 링크 (관리자에게만 출력)</td>
			</tr>
			<tr>
			<th><strong>{{MORE_START}}</strong><br>
			내용<br>
			<strong>{{MORE_END}}</strong></th>
			<td><strong>{{MORE_START}}</strong>와 <strong>{{MORE_END}}</strong> 사이에 들어간 내용 접기</td>
			</tr>
			<tr>
			<th><strong>{{BGM}}</strong></th>
			<td>bgm 컨트롤러 출력</td>
			</tr>
			</tbody>
		</table>
	...
	<tr>
		...
		<td>
		<?=help('pc 내용과 동일하다면 적지 않아도 됩니다.')?>
		<?php echo editor_html('co_mobile_content', get_text($co['co_mobile_content'], 0)); ?></td>
	</tr>
	<tr>
