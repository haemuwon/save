...
$sql_config="update {$g5['config_table']}
				set cf_menu_content = '{$_POST['cf_menu_content']}',
					cf_main_content = '{$_POST['cf_main_content']}',
					cf_footer_content = '{$_POST['cf_footer_content']}',
					cf_logo_use = '{$_POST['cf_logo_use']}'";
...
for($i=0; $i < count($cs_name); $i++) {
	...
	if($de['cs_id']) { 
		// 수정
		if($de['cs_name']!=$cs_name[$i])
			$common=",cs_name = '{$cs_name[$i]}'";
