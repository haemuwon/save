...
if (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc()) { //7.4 호환용 @221024
...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    if ($tmp_mb_id = get_cookie('ck_mb_id')) {
       ...
       // if (strtolower($tmp_mb_id) != strtolower($config['cf_admin'])) {
            $sql = " select mb_password, mb_datetime, mb_intercept_date, mb_leave_date, mb_email_certify from {$g5['member_table']} where mb_id = '{$tmp_mb_id}' ";
            $row = sql_fetch($sql);
            ...
            if ($tmp_key == $key && $tmp_key) {
                ...
                if ($row['mb_intercept_date'] == '' &&
					...
					if (strtolower($tmp_mb_id) == strtolower($config['cf_admin'])) {
						set_session('ss_mb_key', md5($row['mb_datetime'] . $_SERVER['HTTP_USER_AGENT'])); 
					}
                    // 페이지를 재실행
...
if ($bo_table) {
    ...
    if ($board['bo_table']) {
        set_cookie("ck_bo_table", $board['bo_table'], 31);
        $gr_id = $board['gr_id'];
