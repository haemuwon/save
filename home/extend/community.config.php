...
function get_site_content($co_id) {
	...
	if(strstr($pc_str, "{{SETTING}}") || strstr($mo_str, "{{SETTING}}")) { 
		ob_start();
		$str="setting"; 
		include G5_PATH.'/template.php';
		$join_str = ob_get_contents();
		...
		$pc_str = str_replace("{{SETTING}}", $join_str, $pc_str);
		$mo_str = str_replace("{{SETTING}}", $join_str, $mo_str);
	}	
	if(strstr($pc_str, "{{ADMIN}}") || strstr($mo_str, "{{ADMIN}}")) { 
		ob_start();
		$str="admin"; 
		include G5_PATH.'/template.php';
		$join_str = ob_get_contents();
		...
		$pc_str = str_replace("{{ADMIN}}", $join_str, $pc_str);
		$mo_str = str_replace("{{ADMIN}}", $join_str, $mo_str);
	}
	if(strstr($pc_str, "{{MORE_START}}") || strstr($mo_str, "{{MORE_START}}")) { 
		ob_start();
		$str="more_start";
		include G5_PATH.'/template.php';
		$join_str = ob_get_contents();
		...
		$pc_str = str_replace("{{MORE_START}}", $join_str, $pc_str);
		$mo_str = str_replace("{{MORE_START}}", $join_str, $mo_str);
	}

	if(strstr($pc_str, "{{BOX_START}}") || strstr($mo_str, "{{BOX_START}}")) { 
		ob_start();
		$str="div_start";
		include G5_PATH.'/template.php';
		$join_str = ob_get_contents();
		...
		$pc_str = str_replace("{{BOX_START}}", $join_str, $pc_str);
		$mo_str = str_replace("{{BOX_START}}", $join_str, $mo_str);
	}

	if(strstr($pc_str, "{{MORE_END}}") || strstr($mo_str, "{{MORE_END}}") || strstr($pc_str, "{{BOX_END}}") || strstr($mo_str, "{{BOX_END}}")) { 
		ob_start();
		$str="div_end";
		include G5_PATH.'/template.php';
		$join_str = ob_get_contents();
		...
		$pc_str = str_replace("{{MORE_END}}", $join_str, $pc_str);
		$pc_str = str_replace("{{BOX_END}}", $join_str, $pc_str);
		$mo_str = str_replace("{{MORE_END}}", $join_str, $mo_str);
		$mo_str = str_replace("{{BOX_END}}", $join_str, $mo_str);
	}
...
/**
	...
	if(strstr($pc_str, "{{MYPAGE}}") || strstr($mo_str, "{{MYPAGE}}")) { 
		ob_start();
		include G5_PATH.'/template/txt.mypage.php';
...
function emote_ev($comment) {
	...
	for($i=0; $row=sql_fetch_array($emo_result); $i++) { 
		$str = str_replace($row['me_text'], "<img src=\"".G5_URL.$row['me_img']."\">" ,$str); // @211118
	}
	return $str;
}
