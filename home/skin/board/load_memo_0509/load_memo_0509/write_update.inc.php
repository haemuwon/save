...
if($cookie){
	setcookie('MMB_PW', $_POST['wr_password'], 2678400); //@200907
	setcookie('MMB_NAME', $_POST['wr_name'], 2678400); //@200907
}
else {
	setcookie('MMB_PW','');
	setcookie('MMB_NAME','');
	...
	if(!isset($temp['wr_effect'])){
	sql_query(" ALTER TABLE `{$write_table}` ADD `wr_effect` TEXT NOT NULL DEFAULT '' AFTER `wr_url` ");
		}
	if(!isset($temp['wr_back'])){
	sql_query(" ALTER TABLE `{$write_table}` ADD `wr_back` TEXT NOT NULL DEFAULT '' AFTER `wr_url` ");
		}

	unset($temp);
	$customer_sql .= ",wr_reply_more = '{$wr_reply_more}', wr_video = '{$wr_video}', wr_text= '{$wr_text}' ,wr_effect = '{$wr_effect}', wr_tag = '{$wr_tag}', wr_back = '{$wr_back}'";
