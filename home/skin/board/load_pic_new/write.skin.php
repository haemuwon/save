...
if(!$is_error) { 
			...
			<dl>
				...
				<dd>
					...
					<div id="add_VIDEO" <?=$write['wr_type'] == "VIDEO" ? "" : "style='display: none;'"?>>
						<textarea id="wr_video" name="wr_video" class="view_image_area" cols="20" rows="2" placeholder="동영상 소스코드 입력"><?=$write['wr_video']?></textarea>
						<p style="font-size:12px;">* 유튜브 등의 미디어사이트에서 동영상 퍼가기 소스코드를 입력해주세요. *width와 height값 필수</p> 
					</div>
	...
	if ($is_notice || $is_html || $is_secret || $is_mail) {
		...
		$sec .='<option value="member" '.$mem_select.'>멤버공개</option>';}
					...
					<select name="wr_effect" class="effect_select" id="wr_effect" value="1" <?=$write['wr_effect'] == 1 ? "1" : ""?>>
						<option value="">이미지 효과</option>
						<option value="1" <?=$write['wr_effect'] == '1' ? "selected" : ""?>>흰 외곽선</option>
						<option value="2" <?=$write['wr_effect'] == '2' ? "selected" : ""?>>그림자</option>
						<option value="3" <?=$write['wr_effect'] == '3' ? "selected" : ""?>>블러 효과</option>
						<option value="4" <?=$write['wr_effect'] == '4' ? "selected" : ""?>>실루엣 처리</option>
						<option value="5" <?=$write['wr_effect'] == '5' ? "selected" : ""?>>무지개 외곽선</option>
						<option value="6" <?=$write['wr_effect'] == '6' ? "selected" : ""?>>마젠타 외곽선</option>
						<option value="7" <?=$write['wr_effect'] == '7' ? "selected" : ""?>>뽀샤시 효과</option>
					</select>
					<select name="wr_back" class="back_select" id="wr_back" value="1" <?=$write['wr_back'] == 1 ? "1" : ""?>>
						<option value="">배경</option>
						<option value="1" <?=$write['wr_back'] == '1' ? "selected" : ""?>>별(gif)</option>
						<option value="2" <?=$write['wr_back'] == '2' ? "selected" : ""?>>하늘</option>
						<option value="3" <?=$write['wr_back'] == '3' ? "selected" : ""?>>꽃나무</option>
						<option value="4" <?=$write['wr_back'] == '4' ? "selected" : ""?>>창문</option>
						<option value="5" <?=$write['wr_back'] == '5' ? "selected" : ""?>>눈</option>
						<option value="6" <?=$write['wr_back'] == '6' ? "selected" : ""?>>밑줄(텍스트 전용)</option>
						<option value="7" <?=$write['wr_back'] == '7' ? "selected" : ""?>>첨부이미지(이미지 전용)</option>
					</select>
					...
					<dl>
					<dt>
						<label for="wr_tag">이미지 태그</label>
					</dt>
					<dd>
						<input type="text" name="wr_tag" value="<?php if($w=="u"){echo$write['wr_tag'];} ?>" id="wr_tag" class="frm_input" size="50">
					</dd>
				</dl>	
			</div>
			<hr class="padding small" /> 
