	...
	if(!$log_comment['wr_noname']) { 
		if(is_file(G5_DATA_PATH."/site/ico_admin") && $config['cf_admin'] == $log_comment['mb_id']) {
			...
			if(is_file($log_icon_path) && $log_comment['mb_id']) {
				// 멤버 아이콘이 존재하고 작성자와 멤버가 동일할 경우
				$log_comment['ch_name'] = "<img src='".G5_DATA_URL."/member/".substr($log_comment['mb_id'],0,2)."/".$log_comment['mb_id'].".gif' style='height:15px; padding-right:5px;'/>";
			}
			else {
				$log_comment['ch_name'] = "";
			} 
...
<div class="<?=$cmt_depth>0 ? "item-reply ":"";?><?=($has_child['wr_id']&&($has_child['wr_id']!=$log_comment['wr_id'])&&$cmt_depth==0) ? "parent " : "";?><?=$has_child['wr_id']==$log_comment['wr_id'] ? "last ": "";?>item-comment" id="c_<?php echo $comment_id ?>" style="padding-left:<?=$cmt_depth>0 ? "6px":"0px";?>;">
	<div class="co-header">
		<? // 로그 작성자와 코멘트 작성자가 동일할 경우, class='owner' 를 추가한다. ?>
		<? if(!$log_comment['wr_noname']) {  
		?>
		<span class="font3"><p2 <?=$is_comment_owner ? ' class="owner"' : ''?>>
			<?=$comment_owner_front?>
			<?=$log_comment['ch_name'].$log_comment['name']?>
			<?=$comment_owner_behind?>
		</p></span>
		<? } else { ?>
			<span class="font3"><p>익명의 누군가</p></span>
		<? } ?>
	<span class="date">
<?=date('m.d | H:i', strtotime($log_comment['wr_datetime']))?>
	...
	<div class="co-content">
		<?if($log_comment['wr_1']){?>
			<p><span class="co-memo highlight-holo"><?=$log_comment['wr_1']?></span></p>
		<?}?>
		<?if(strstr($log_comment['wr_option'],"secret")){?>
			<span class="co-secret sub2">비밀글</span><br>
		<?}else if ($log_comment['wr_secret']==1){?>
			<span class="co-member sub2">로그인 전용</span><br>
		<?} 
			if ($log_comment['wr_reply_more']==1){?>
			<a href="#" class="co-more highlight2"><span>펼치기</span></a>
			<?} 
		?>
		<div class="original_comment_area<?=$log_comment['wr_reply_more'] ? " re_more":"";?>">
			...
			if($log_comment['wr_type']=="UPLOAD" && $log_comment['wr_is_comment']!=0 && $log_comment['c_view']==true){
				...
				<p><img src="<?=$thumb['src']?>"  style="border-radius:15px; margin:4px auto;" onclick="window.open(this.src)" style="cursor:pointer;" class="<?=$pic_class?> <?=$gray?>"></p>
			<?}
		...
		<div class="modify_area" id="save_comment_<?php echo $comment_id ?>"> 
			...
			<input type="hidden" id="save_effect_<?php echo $comment_id ?>" value="<?=$log_comment['wr_effect']?>" name="wr_effect" class="re-effect"/>
			<select name="wr_effect" class="re-effect id="save_effect_<?php echo $comment_id ?>"?>/>
			<option value="1" <?if(strstr($log_comment['wr_effect']==1)) echo "selected";?>>흰 외곽선</option>
			<option value="2" >그림자</option>
			<option value="3">블러 효과</option>
			<option value="4" >실루엣 처리</option>
			<option value="5">무지개 외곽선</option>
			<option value="6" >마젠타 외곽선</option>
			</select>
			</p>
		</div>
		<? } ?>
