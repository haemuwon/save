...
if(!sql_query(" SELECT bd_name from {$g5['character_body_table']} ")) {
	sql_query(" ALTER TABLE `{$g5['character_body_table']}` ADD `bd_name` VARCHAR(255) NOT NULL AFTER `wr_id` ");
}
if(!sql_query(" SELECT bd_secret from {$g5['character_body_table']} ")) {
	sql_query(" ALTER TABLE `{$g5['character_body_table']}` ADD `bd_secret` int(11) NOT NULL default '0' AFTER `bd_name` ");
}