...
<div class="<?=$cmt_depth>0 ? "item-reply ":"";?><?=($has_child['wr_id']&&($has_child['wr_id']!=$log_comment['wr_id'])) ? "parent " : "";?><?=$has_child['wr_id']==$log_comment['wr_id'] ? "last ": "";?>item-comment" id="c_<?php echo $comment_id ?>" style="padding-left:<?=$cmt_depth?>px;">
	...
	<div class="co-content">
		<!-- 메모 -->
		<?if($log_comment['wr_1']){?>
			<p><span class="co-memo mr-highlight"><?=$log_comment['wr_1']?></span></p>
		<?}?>

		<!-- 펼치기 버튼 -->
		<? if ($log_comment['wr_reply_more']==1){?>
			<a href="#" class="co-more mr-highlight2-outline"><span>펼치기</span></a>
		<?}?>

		<!-- 공개범위 -->
		<? if(strstr($log_comment['wr_option'],"secret") && $is_admin){ ?>
			<span class="co-secret mr-highlight4">#비밀글</span>
		<? }else if ($log_comment['wr_secret'] && $is_member){ ?>
		...
		<div class="original_comment_area<?=$log_comment['wr_reply_more'] ? " re_more":"";?>">
			...
			if($log_comment['wr_type']=="UPLOAD" && $log_comment['wr_is_comment']!=0 && $log_comment['c_view']==true){
				...
				<p>
					<a class="log-<?=$log_comment['wr_id']?>-image-link" href="<?=$thumb['src']?>" data-lightbox="<?=$log_comment['wr_id']?>">
						<img class="log-<?=$log_comment['wr_id']?>-image" src="<?=$thumb['src']?>" style="cursor:pointer;">
					</a>
				</p>
			<?}
	...
	<div class="co-footer">
		...
		<? if($log_comment['wr_is_comment']!=0){?>
			<? if ($log_comment['is_del'])  { ?><a href="<?php echo $log_comment['del_link'];  ?>" onclick="return comment_delete();" class="del">삭제</a><?php } ?>
			<? if ($log_comment['is_edit']) { ?><a href="<?php echo $c_edit_href;  ?>" onclick="comment_box('<?=$list_item['wr_id']?>','<?php echo $comment_id ?>', 'cu'); return false;" class="mod">수정</a><?php } ?>
