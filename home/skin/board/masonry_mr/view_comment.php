...
{ 
	...
	if (
		(!strstr($c_row['wr_option'], 'secret') && !$c_row['wr_secret']) ||
		($c_row['wr_secret'] && $member['mb_id']) ||
		$is_admin ||
		...
		if(!get_session($ss_name)){
			...
			if($comment[$c_i]['mb_id']!='') 
				if(!strstr($c_row['wr_option'], 'secret') && $c_row['wr_secret'])
					$comment[$c_i]['content'] = '<span class="mr-highlight4">멤버공개 댓글입니다.</span>';
				else 
					$comment[$c_i]['content'] = '<span class="mr-highlight4">비공개 댓글입니다.</span>';
			else 
